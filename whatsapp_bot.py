...
@app.get("/ping")
async def ping():
    return {"status": "ok", "message": "Meka bot is alive 🚀"}
...
async def whatsapp_reply(From: str = Form(...), Body: str = Form(...)):
    ...
    # If waiting for goal    
        ...
        user.state = "idle"
        db.commit()
        msg.body(f"✅ Got it, {user.name}! Your goal is: \n{user.goal}.\n\n"
    ...
    # --- COMMANDS ---
        ...
        msg.body(f"👋 Hey {user.name or 'friend'}, welcome back!\n"
        ...
        if not entries:
            msg.body("🗒 No history yet. Log progress with 'progress'.")
        else:
        ...
        if user.streak >= 30:
            ...
            msg.body(f"🚫 Not yet! You need a 30-day streak. Current streak: {user.streak}")
